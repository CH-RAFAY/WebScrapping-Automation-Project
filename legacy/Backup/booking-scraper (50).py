...
def run(playwright: Playwright) -> None:
    ...
    # Modified to accommodate more results
    for _ in range(3):  # Scroll multiple times to load more results
        page.press("body", "PageDown")
        page.wait_for_timeout(1000)

    def getHotelsUrl():
        urls = page.query_selector_all("xpath=//html/body/div[12]/div[2]/div/div[2]/div/div/ul/li[.]")
        # Modified to get 50 URLs instead of 10
        for i, urls_links in enumerate(urls[:50]):
            hotels_url = urls_links.query_selector('a')
            if hotels_url and len(hotels) < 50:  # Changed limit to 50
                hotels.append(hotels_url.get_attribute("href"))
                print(f"Added hotel URL {i + 1}/50")  # Updated progress message
...
async def info():
    for i, links in enumerate(hotels):
        print(f"\nScraping hotel {i + 1}/50")  # Updated progress message
...
print("\nFinal Dataset:")
print(df)
df.to_csv('hotels_top50.csv', index=False)  # Updated filename to reflect 50 hotels
